...
AForm::AForm(std::string const &name, int signGrade, int executeGrade, std::string const &target)
    : _name(name), _signed(false), _signGrade(signGrade), _executeGrade(executeGrade), _target(target)
{
    if (signGrade < HIGHEST_GRADE || executeGrade < HIGHEST_GRADE)
        throw GradeTooHighException();
    else if (signGrade > LOWEST_GRADE || executeGrade > LOWEST_GRADE)
